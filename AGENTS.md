...
| `aws_deployer` | AWS SDK/CLI (boto3 wrapper) | Provision & deploy infrastructure/app | `invoke(action: str, params: dict)` where `action` ∈ {`launch_ec2`, `stop_ec2`, `terminate_ec2`, `list_ec2_instances`, `create_bucket`, `describe_images`, `describe_key_pairs`, `upload_s3`, `download_s3`, `list_s3_objects`, `deploy_lambda`, `update_lambda_code`, `invoke_lambda`, `create_cluster`, `register_task_definition`, `create_service`, `update_service`} | Credentials pulled just-in-time from MongoDB (never `.env`); destructive actions require explicit confirmation; dry-run (`--no-execute` or equivalent) before apply when supported |
